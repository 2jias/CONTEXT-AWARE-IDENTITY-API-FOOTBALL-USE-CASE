...
export default function CoachDashboard() {
  ...
  const load = async (id) => {
    const { data } = await api.get(`/player/${id}?context=club`);
    setInfo(data);
  };

  return (
    <Layout>
      <h2 className="page-title">Welcome Coach!</h2>
      <section className="grid grid-2">
        ...
        <aside className="card card-pad">
          ...
          <p className="mb-16">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec volutpat ultrices magna.</p>
          <p className="mb-16">Pellentesque at mauris nunc. Donec quis sapien pellentesque accumsan eu rutrum nunc.</p>
        </aside>
      </section>
    </Layout>
  );
